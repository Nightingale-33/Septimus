...
export class FillAction extends ReservingAction<ResourceReservation> {
  ...
  run(creep: Creep): boolean {
    ...
    if (this.pos) {
      ...
      moveTo(creep, { pos: this.pos, range: 1 }, { priority:500 + 1000 / range, avoidCreeps: avoidCreeps, visualizePathStyle:{stroke:"#2626FF"}, roomCallback:MovementRoomCallback, swampCost:5, plainCost:2 },{avoidCreeps:false,priority:500+ 1000*range});
    }
  ...
  ApproxTimeLeft(creep: AbstractCreep): number {
    let expected = 1;
    let travel = Math.max(this.pos.getMultiRoomRangeTo(creep.pos),1) - 1;
